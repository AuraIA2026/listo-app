      ...
      <div onClick={onClose} style={{
        ...
        <div onClick={e => e.stopPropagation()} style={{
          ...
          <div className="uu-scroll" style={{ flex: 1, overflowY: 'auto', padding: '8px 16px 30px', scrollbarWidth: 'none' }}>
            ...
            <Accordion title={lang === 'es' ? '💼 Trabajar como Profesional' : '💼 Work as a Professional'} open={open === 'postularse'} onToggle={() => toggle('postularse')}>
              {!profileComplete ? (
                ...
                <div style={{ textAlign: 'center', padding: '10px 0' }}>
                  <p style={{ fontSize: '13px', color: '#666', marginBottom: '16px', lineHeight: 1.5, textAlign: 'left' }}>
                    {lang === 'es'
                      ? '¡Únete a nuestra red de profesionales y comienza a recibir trabajos! Te regalaremos el Plan Básico con 3 contratos gratis al mes durante tus primeros 3 meses de registro.'
                      : 'Join our professional network and start receiving jobs! We will gift you the Basic Plan with 3 free contracts per month for your first 3 months.'}
                  </p>
                  <button 
                    onClick={() => {
                    ...
                    style={{ background: '#F26000', color: '#fff', border: 'none', borderRadius: '10px', padding: '12px 20px', fontSize: '14px', fontWeight: '800', cursor: 'pointer', width: '100%', boxShadow: '0 4px 10px rgba(242,96,0,0.2)' }}
                  >
                    🚀 {lang === 'es' ? 'Iniciar Postulación' : 'Start Application'}
                  </button>
                </div>
              )}
            </Accordion>
